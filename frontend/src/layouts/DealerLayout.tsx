...
const DealerLayout: React.FC = () => {
  ...
  const menuItems = [
    { path: '/dealer/dashboard', label: 'Dashboard', icon: LayoutDashboard, badge: null, desc: 'Overview & analytics' },
    ...
    { path: '/dealer/batches', label: 'Batch Tracking', icon: Search, badge: null, desc: 'Traceability status' },
    { path: '/dealer/reports', label: 'Reports', icon: ClipboardList, badge: null, desc: 'Operations reports' },
    { path: '/dealer/profile', label: 'Profile', icon: UserIcon, badge: null, desc: 'Account details' }
  ];
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row" style={{ background: 'linear-gradient(135deg, #0a0f1a 0%, #0d1f17 50%, #0a0f1a 100%)' }}>
      ...
      <aside className="hidden md:flex md:w-72 flex-col justify-between shrink-0 relative z-20 border-r border-white/5"
          ...
          <div className="px-6 py-7 border-b border-white/5">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <span className="text-[9px] font-bold uppercase tracking-[0.15em] text-emerald-400/70">Warehouse Portal</span>
              </div>
            </div>
          </div>
              ...
              return (
                <Link key={item.path} to={item.path}
                  className={`relative flex items-center gap-3 px-3.5 py-3 rounded-xl text-sm font-bold transition-all duration-200 group overflow-hidden ${
                    isActive ? 'text-white' : 'text-slate-400 hover:text-white hover:bg-white/4'
                  }`}>
                  {isActive && (
                    <motion.div layoutId="activeDealerNav" className="absolute inset-0 rounded-xl border border-emerald-500/30"
        ...
        <div className="relative z-10 p-4 border-t border-white/5">
          ...
          <div className="grid grid-cols-2 gap-2">
            <Link to="/" className="flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold text-slate-300 border border-white/8 hover:border-emerald-500/30 hover:text-emerald-400 transition-all bg-white/3">
              <Home size={12} /> Portal
            </Link>
            <button onClick={handleLogout}
              className="flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold text-rose-400 border border-rose-500/15 hover:bg-rose-500/10 transition-all bg-white/3">
              <LogOut size={12} /> Exit
            </button>
          </div>
        </div>
      ...
      {/* ===== MOBILE TOP BAR ===== */}
      <div className="md:hidden flex items-center justify-between p-4 border-b border-white/5 relative z-30 bg-[#080d16]">
        ...
        {mobileMenuOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed top-0 left-0 bottom-0 w-72 z-20 p-5 flex flex-col justify-between shadow-2xl md:hidden border-r border-white/8 bg-[#080d16] text-white"
            >
              <div>
              ...
              <div className="pt-4 border-t border-white/8">
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-8 h-8 rounded-xl bg-emerald-50/15 border border-emerald-50/25 flex items-center justify-center"><UserIcon size={14} className="text-emerald-400" /></div>
                  <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <Link to="/" onClick={() => setMobileMenuOpen(false)} className="flex items-center justify-center gap-1.5 py-2 bg-white/5 border border-white/8 rounded-xl text-xs font-bold text-white"><Home size={12} /> Portal</Link>
      ...
      <div className="flex-1 flex flex-col min-h-0 min-w-0">
        ...
        {/* Top Header Bar */}
        <header className="hidden md:flex justify-between items-center px-8 py-4 border-b border-white/5 relative z-10 shrink-0 bg-[#090e18]">
          <div>
            <div className="flex items-center gap-2 mb-0.5">
              <Activity size={14} className="text-emerald-400" />
              <span className="text-[9px] font-black text-emerald-400 uppercase tracking-[0.15em]">B2B Telemetry</span>
            </div>
            <h1 className="text-xl font-black text-white">Welcome Back, <span className="text-transparent bg-clip-text" style={{ backgroundImage: 'linear-gradient(135deg,#10B981,#34d399)' }}>{user?.name || 'Dealer'}</span></h1>
          </div>

          <div className="flex items-center gap-3">
            {/* Warehouse indicator */}
            <div className="flex items-center gap-2 text-xs font-bold px-4 py-2 rounded-xl border border-white/8 text-slate-300 bg-white/5">
            ...
            <div className="relative">
              <button onClick={() => setNotifOpen(!notifOpen)}
                className="relative w-9 h-9 flex items-center justify-center rounded-xl border border-white/8 text-slate-400 hover:text-white hover:border-emerald-500/30 transition-all bg-white/5">
                <Bell size={16} />
            ...
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl border border-emerald-500/20 bg-emerald-500/10">
              ...
              <span className="text-[10px] font-black text-emerald-400">TELEMETRY LIVE</span>
            </div>
          </div>
        </header>

        {/* Content Outlet */}
        <main className="flex-1 p-6 md:p-8 overflow-y-auto custom-scrollbar bg-[#070b12]">
