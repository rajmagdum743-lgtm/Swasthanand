...
const DealerTrace: React.FC = () => {
  ...
  // ── Audit by typing a batchId / SKU / productId manually ──
  const handleAudit = async (idToQuery?: string) => {
    ...
    try {
      // Strict priority: batchId first, then id, then sku — avoids cross-matching
      const product =
  ...
  // ── Audit directly from a chip click — bypasses re-lookup entirely ──
  const handleAuditForProduct = async (product: any) => {
    ...
    try {
      ...
      // Use the exact product object that was clicked — no re-lookup
      setAuditResult({
        product,
        batch: batch || {
          harvestDate: product.harvestDate || '2024-05-20',
