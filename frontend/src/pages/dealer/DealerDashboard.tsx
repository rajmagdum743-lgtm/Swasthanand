...
const DealerDashboard: React.FC = () => {
  ...
  const lowStockProducts = dealerProducts.filter((p: any) => (p.stock ?? 0) < 15);
  ...
  const triggerProductScan = (product: any) => {
    setIsScanning(true);
    setScanResult(null);
    // Play scan beep using Web Audio API
    try {
        ...
        {scannerOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm">
            ...
            >
              ...
              <div className="aspect-video bg-black rounded-xl border border-slate-800 relative overflow-hidden flex flex-col items-center justify-center p-4 mb-4">
                {isScanning ? (
                  ...
                  <div className="text-center text-slate-400 space-y-2 z-10">
                    ...
                    <p className="text-xs font-bold">Align product barcode or select an item below to scan</p>
                    <button
                      onClick={startCamera}
              ...
              {!isScanning && (
                <div className="space-y-3">
                  <span className="block text-[8px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Select Product to Scan</span>
